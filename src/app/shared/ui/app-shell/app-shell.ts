...
/**
 * The routed layout: skip link, header, left navigation, offline banner, the outlet, the toast
 * stack and the one live region.
 *
 ...
 *
 * WEB-UX-031 — no horizontal page scroll at 360 px: `<main>` is a flexed column beside the nav,
 * with padding and no fixed width. Anything genuinely wide (the queue table) scrolls inside its
 * own container, which is the feature's job.
 *
 * WEB-UX-040 — the skip link is the first tab stop on every route, and `<main>` takes focus
 * when it is used.
 *
 * The mobile nav drawer's open/closed state lives here rather than in `SideNav` itself, because
 * the button that opens it (the header's hamburger) and the panel it opens are siblings, not
 * parent/child — the shell is the nearest common owner. `SideNav`'s OWN icon-rail collapse is a
 * separate, purely-visual choice it keeps to itself.
 */
...
@Component({
  ...
  imports: [RouterOutlet, TranslatePipe, AppHeader, SideNav, OfflineBanner, ToastHost, LiveRegion],
...
export class AppShell {
  ...
  protected readonly navOpen = this._navOpen.asReadonly();

  protected toggleNav(): void {
    this._navOpen.set(!this._navOpen());
  }
